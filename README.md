...
<img width="1803" height="953" alt="Screenshot from 2026-02-27 18-13-33" src="https://github.com/user-attachments/assets/b46c90fd-2046-4546-a752-53c9798b463b" />
...
| Pattern | Matches |
|---------|---------|
| `s_*_g` | All columns matching `s_0_g`, `s_1_g`, `s_2_g`, ... |
| `x?` | `x1`, `x2`, ... but not `x10` |
| `feature_[0-9]*` | Columns starting with `feature_` followed by digits |
...
### Example: High-Dimensional (DeepONet)

```csv
s_0_g,s_1_g,...,s_63_g,query_x,query_y,u
0.12,0.34,...,0.56,1.0,2.0,0.789
```

Saved config uses glob: `input_columns: s_*_g,query_x,query_y` / `target_columns: u`
...
- `query_x, query_y` — exterior query point coordinates
...
2. Load the "DeepONet" saved graph from the toolbar
3. Upload `laplace_train.csv` via the config panel
4. Select all `s_*_g` columns + `query_x` + `query_y` as inputs, `u` as target (or keep the saved config which uses glob patterns)
5. Click Execute
